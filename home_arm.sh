#!/bin/bash
# Homes the follower arm to a safe position smoothly
# Uses servo internal acceleration for smooth motion
# Usage: bash home_arm.sh [acceleration] (lower = smoother, default 5)
...
for _ in range(50):
    ...
    if all(abs(current[k] - HOME[k]) < 1.0 for k in HOME):
        break
    time.sleep(0.2)
